...
// For Bound actions on SalesOrders
...
this.after('READ', 'PurchaseOrders', (results) => {
  ...
  for (const p of results) {
    ...
    if (p.status === 'Cancelled') p.statusCriticality = 1;        // Red

    else if (p.status === 'Pending') p.statusCriticality = 2;    // Orange

    else p.statusCriticality = 3;                       // Green
